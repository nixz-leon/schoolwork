...
// this template gets the length of a matrix, its pretty neat can explain it during the interview if  needed, used it in code previously
...
int main(){
    bool exitcond = true;
    string a = "Calculate the Likeness Score between 2 strings of equal length.";
    string b = "Calculate the Best Likeness Score between 2 sequences of possibly different length.";
    string c = "Find winner among sequences of 3 players and a Golden Sequence.";
    string d = "Exit.";
    string options[4] = {a , b, c, d};
    string submenu1base = "Enter Sequence";
    string option3options[4] = {"Enter Sequence of Player 1", "Enter Sequence of Player 2", "Enter Sequence of Player 3", "Enter Golden sequence"};
    while(exitcond){
        int choice;
        cout << "----Menu!----" << endl;
        int count;
        for(count = 1; count <= len(options); count++){
            cout << "Choice " << count << ": " << options[(count-1)] << endl;
        }
        cout << "Enter your choice (1-" << (count-1) <<  "):" << endl;
        cin >> choice;
        // How it works: since i am exressing the options as a matrix composed of strings, it goes through the matrix with a counter formating the number of options
        // it will paste in the correct number option for the menu item, as well as saves on space in the actual document, while maintaining complexity i believe.
        // this technically allows for the ablity to alter the code better going forward also its neat
        if(choice == (count-1)){
            exitcond = false;
            cout << "Exiting." << endl;
        }else if(choice > 0 && choice < count){
            if(choice == 1){
                //this is for choice one, needs to check for equal length
                string seq1, seq2;
                cout << submenu1base << " 1" << endl;
                cin >> seq1;
                cout << submenu1base << " 2" << endl;
                cin >> seq2;
                ...
                //for the second choice and needs to check that one string is greater than the others
                string seq1, seq2;
                cout << submenu1base << " 1" << endl;
                cin >> seq1;
                cout << submenu1base << " 2" << endl;
                ...
                // the one with three players and needs to check that all the player seqs are better than golden seq
                string seq1, seq2, seq3, seq4;
                cout << option3options[0] << endl;
                cin >> seq1;
                cout << option3options[1] << endl;
                cin >> seq2;
                cout << option3options[2] << endl;
                cin >> seq3;
                cout << option3options[3] << endl;
                cin >> seq4;
                if (seq1.size() >= seq4.size() && seq2.size() >= seq4.size() && seq3.size() >= seq4.size()){
                    findWinner(seq1, seq2, seq3, seq4);
                }
            }
        }else{
            cout << "Invalid Input. Choices between 1-" << (count-1) << "." << endl;
        }
    }
}